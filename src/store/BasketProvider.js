...
const basketReducer = (state, action) => {
  if (action.type === 'ADD') {
    const updatedTotalAmount =
      state.totalAmount + action.item.unitPrice * action.item.amount;
    ...
    if (existingBasketItem) {
        const updatedItem = {
          ...existingBasketItem,
          amount: existingBasketItem.amount < action.item.stockCount ? existingBasketItem.amount + 1 : existingBasketItem.amount,
        };
